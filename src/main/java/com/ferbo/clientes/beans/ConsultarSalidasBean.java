...
public class ConsultarSalidasBean implements Serializable {
	...
	public ConsultarSalidasBean() {
		...
		this.listSalidas = new ArrayList<Salida>();
		status = new ManageStatus();

		String mensaje = String.format("Usuario %s ingresa a Consulta de orden de salida.",
				this.cteContacto.getUsuario());
		log.info(mensaje);
	}
	...
	public void consultarSalidas() {
		...
		try {
			...
			this.listSalidas = this.consultaSalidasBL.buscarPorClientePeriodo(this.cliente.getIdCliente(),
					DateUtils.toLocalDate(this.fechaInicio), DateUtils.toLocalDate(this.fechaFin));
	...
	public void obtenerReporte() {
		...
		try {
			...
			if (horaSalida == null)
				horaSalida = DateUtils.toDate(salidaSelected.getHoraSalida());

			if (fechaSalida == null)
				fechaSalida = DateUtils.toDate(salidaSelected.getFechaSalida());
			...
			for (Integer idPlanta : alPlantas) {
				...
				this.filePDF = DefaultStreamedContent.builder().contentType("application/pdf").name(filename)
						.stream(() -> input).build();
	...
	public Boolean puedeCancelar() {
		...
		if (this.salidaSelected.getFechaSalida().isBefore(LocalDate.now()))
			return Boolean.FALSE;

		switch (this.salidaSelected.getStatus().getClave()) {
		case "A":
		case "C":
			return Boolean.FALSE;
		}
	...
	public void cancelarOrden() {
		...
		try {
			if (this.salidaSelected == null)
				throw new ClientesException(
						"Error: folio de salida no encontrado. Si el problema persiste, contacte al administrador del sistema");
			...
			mensaje = String.format("Se envió un correo con la solicitud de cancelación del folio %s",
					this.salidaSelected.getFolio());
			severity = FacesMessage.SEVERITY_INFO;
		} catch (Exception ex) {
			log.error("Problema general...", ex);
			...
			message = new FacesMessage(severity, "Cancelación solicitada", mensaje);
			FacesContext.getCurrentInstance().addMessage(null, message);
		}
	}
