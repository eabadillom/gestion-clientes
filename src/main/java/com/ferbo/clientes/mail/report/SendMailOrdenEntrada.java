...
public class SendMailOrdenEntrada extends Thread{
	...
	public SendMailOrdenEntrada(Connection conn) {
		this.conn = conn;
		this.alAdjuntos = new ArrayList<Adjunto>();
	}
	
	public SendMailOrdenEntrada() {
		this.alAdjuntos = new ArrayList<Adjunto>();
	}
	...
	public void exec(String folio) {
		List<Ingreso> detOrden = null;
		Integer planta = null;
		List<IngresoProducto> ingresosProducto = null;
		Boolean isHorarioEspecial = null;
		Date horaEntrada = null;
		Date horaLimite = null;

		try {
			detOrden = IngresoDAO.getIngresosFolio(conn,folio);			
			
			for (Ingreso orden : detOrden) {

				if (horaEntrada == null) {
					horaEntrada = new Date(orden.getFechaHora().getTime());
					horaLimite = new Date(horaEntrada.getTime());
					DateUtils.setHora(horaLimite, 17);
					DateUtils.setMinuto(horaLimite, 0);
					DateUtils.setSegundo(horaLimite, 0);
					DateUtils.setMilisegundo(horaLimite, 0);
					
				}
				
				if (idCliente == null)
					idCliente = new Integer(orden.getIdCliente());							
				
				ingresosProducto = IngresoProductoDAO.getIngresosProducto(conn, orden);
				
				planta = ingresosProducto.get(0).getIdPlanta(); 
							
				
			}

			if (horaEntrada.getTime() >= horaLimite.getTime())
				isHorarioEspecial = new Boolean(true);

			
			this.processReport(conn, folio, planta, isHorarioEspecial);
	...
	private void processReport(Connection conn, String folio, Integer idPlanta, Boolean isHorarioEspecial) {
		...
		try {
			reportNameJASPER = "/jasper/SolicitudEntradaMercancia.jrxml";
			...
			parameters.put("REPORT_TIME_ZONE", TimeZone.getTimeZone("GMT-6:00"));

			subject = String.format("Orden de entrada %s", folio);
			...
			mailInventarioHTML = "/mail/mailOrdenEntrada.html";
			...
			Adjunto attachment = new Adjunto("OrdenEntrada" + folio + ".pdf", Adjunto.TP_ARCHIVO_PDF, binContenidoOrdenSalida);
